 ...
 * plugin until absolutely necessary (if ever). Additionally, LazyMetadataValue
 * objects cache their values internally unless overridden by a
 * {@link CacheStrategy} or invalidated at the individual or plugin level. Once
 * invalidated, the LazyMetadataValue will recompute its value when asked.
 */
public class LazyMetadataValue extends MetadataValueAdapter implements MetadataValue {
	...
	/**
	 * Protected special constructor used by FixedMetadataValue to bypass standard
	 * setup.
	 * 
	...
	public enum CacheStrategy {
		/**
		 * Once the metadata value has been evaluated, do not re-evaluate the value
		...
		/**
		 * Once the metadata value has been evaluated, do not re-evaluate the value in
		 * spite of manual invalidation.
		 */
		CACHE_ETERNALLY
	}
}
