 ...
 * <p>
 * Complex implementations can be created by overriding the behavior of all the
...
public abstract class HelpTopic {
	...
	/**
	 * Allows the server administrator to override the permission required to see a
	 * help topic.
	 * <p>
	 * HelpTopic implementations should take this into account when determining
	 * topic visibility on the
	 * {@link HelpTopic#canSee(org.bukkit.command.CommandSender)} function.
	 *
	 * @param amendedPermission The permission node the server administrator wishes
	...
	/**
	 * Returns the full description of this help topic that is displayed when the
	 * user requests this topic's details.
	 * <p>
	 * The result will be paginated to properly fit the user's client.
	 *
	 * @param forWho The player or console requesting the full text. Useful for
	 *               further security trimming the command's full text based on
	...
	/**
	 * Allows the server admin (or another plugin) to add or replace the contents of
	 * a help topic.
	 * <p>
	 * A null in either parameter will leave that part of the topic unchanged. In
	 * either amending parameter, the string {@literal <text>} is replaced with the
	 * existing contents in the help topic. Use this to append or prepend additional
	 * content into an automatically generated help topic.
	 *
	 * @param amendedShortText The new topic short text to use, or null to leave
	 *                         alone.
	...
	/**
	 * Developers implementing their own custom HelpTopic implementations can use
	 * this utility method to ensure their implementations comply with the expected
	 * behavior of the {@link HelpTopic#amendTopic(String, String)} method.
	 *
	 * @param baseText  The existing text of the help topic.
	 * @param amendment The amending text from the amendTopic() method.
	 * @return The application of the amending text to the existing text, according
	 *         to the expected rules of amendTopic().
	 */
