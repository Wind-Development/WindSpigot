 ...
 * in the graph. Since each Prompt chooses the next Prompt, complex conversation
 * trees can be implemented where the nature of the player's response directs
 ...
 * <p>
 * Each conversation has a timeout measured in the number of inactive seconds to
 * wait before abandoning the conversation. If the inactivity timeout is
 * reached, the conversation is abandoned and the user's incoming and outgoing
 * chat is returned to normal.
 * <p>
 * You should not construct a conversation manually. Instead, use the
 * {@link ConversationFactory} for access to all available options.
 */
public class Conversation {
	...
	/**
	 * Sets the modality of this conversation. If a conversation is modal, all
	 * messages directed to the player are suppressed for the duration of the
	 * conversation.
	 *
	...
	/**
	 * Displays the first prompt of this conversation and begins redirecting the
	...
	/**
	 * Passes player input into the current prompt. The next prompt (as determined
	 * by the current prompt) is then displayed to the user.
	 *
	...
	/**
	 * Abandons and resets the current conversation. Restores the user's normal chat
	 * behavior.
	 */
	public void abandon() {
