...
public class EntityTargetEvent extends EntityEvent implements Cancellable {
	 ...
	 * <p>
	 * This will be null in the case that the event is called when the mob forgets
	 * its target.
	 *
	 * @return The entity
	 */
	 ...
	 * <p>
	 * This is different from cancelling the event. Cancelling the event will cause
	 * the entity to keep an original target, while setting to be null will cause
	 * the entity to be reset.
	 *
	...
	public enum TargetReason {
		...
		/**
		 * When the target attacks a fellow pig zombie, so the whole group will target
		 * him with this reason.
		 */
		PIG_ZOMBIE_TARGET,
		/**
		...
		/**
		 * When the owner of the entity attacks the target attacks, so the entity
		 * targets it.
		 */
		...
		DEFEND_VILLAGE,
		/**
		 * When the target attacks a nearby entity of the same type, so the entity
		 * targets it
		 */
		TARGET_ATTACKED_NEARBY_ENTITY,
		/**
		 * When a zombie targeting an entity summons reinforcements, so the
		 * reinforcements target the same entity
		 */
