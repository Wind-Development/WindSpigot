 ...
 * {@link BukkitScheduler#runTask(Plugin, Runnable)}, which will run the task on
 * the next tick. Also be aware that this is not an exhaustive list, and other
 * methods could potentially create issues as well.
 * <p>
...
public class InventoryClickEvent extends InventoryInteractEvent {
	...
	/**
	 * Gets whether the ClickType for this event indicates that the key was pressed
	 ...
	 * @deprecated This changes the ItemStack in their hand before any calculations
	 *             are applied to the Inventory, which has a tendency to create
	 *             inconsistencies between the Player and the server, and to make
	 *             unexpected changes in the behavior of the clicked Inventory.
	 */
	@Deprecated
	 ...
	 * {@link InventoryView #getItem(int)} This slot number is unique for the view.
	 *
	 * @return the slot number
	 */
	 ...
	 * pressed key (0-8).
	 *
	 * @return the number on the key minus 1 (range 0-8); or -1 if not a NUMBER_KEY
	 *         action
	 */
	 ...
	 * <p>
	 * This action cannot be changed, and represents what the normal outcome of the
	 * event will be. To change the behavior of this InventoryClickEvent, changes
