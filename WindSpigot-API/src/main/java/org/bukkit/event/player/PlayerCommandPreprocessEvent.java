 ...
 * <code>${nearbyPlayer}</code> with the name of the nearest other player, or
 * simulating the <code>@a</code> and <code>@p</code> decorators used by Command
 * Blocks in plugins that do not handle it.
 * <li>Conditionally blocking commands belonging to other plugins. For example,
 * blocking the use of the <code>/home</code> command in a combat arena.
 * <li>Per-sender command aliases. For example, after a player runs the command
 * <code>/calias cr gamemode creative</code>, the next time they run
 * <code>/cr</code>, it gets replaced into <code>/gamemode creative</code>.
 ...
 * beginning of the message should be preserved. If a slash is added or removed,
...
public class PlayerCommandPreprocessEvent extends PlayerEvent implements Cancellable {
	 ...
	 * <p>
	 * All commands begin with a special character; implementations do not consider
	 * the first character when executing the content.
	 *
	 ...
	 * <p>
	 * The set returned is not guaranteed to be mutable and may auto-populate on
	 * access. Any listener accessing the returned set should be aware that it may
	 * reduce performance for a lazy set implementation. Listeners should be aware
	 * that modifying the list may throw {@link UnsupportedOperationException} if
	 * the event caller provides an unmodifiable set.
	 *
	 * @deprecated This method is provided for backward compatibility with no
