...
public interface PlayerProfile extends Cloneable, ConfigurationSerializable {
	 ...
	 * <p>
	 * A profile is currently considered complete if it has a name, a unique id, and
	 * textures.
	 *
	 ...
	 * <p>
	 * This tries to produce a completed profile by filling in missing properties
	 * (name, unique id, textures, etc.), and updates existing properties (e.g.
	 * name, textures, etc.) to their official and up-to-date values. This operation
	 * does not alter the current profile, but produces a new updated
	 * {@link PlayerProfile}.
	 * <p>
	 * If no player exists for the unique id or name of this profile, this operation
	 * yields a profile that is equal to the current profile, which might not be
	 * complete.
	 * <p>
	 * This is an asynchronous operation: Updating the profile can result in an
	 * outgoing connection in another thread in order to fetch the latest profile
	 * properties. The returned {@link CompletableFuture} will be completed once the
	 * updated profile is available. In order to not block the server's main thread,
	 * you should not wait for the result of the returned CompletableFuture on the
	 * server's main thread. Instead, if you want to do something with the updated
	 * player profile on the server's main thread once it is available, you could do
	 * something like this:
	 * 
	 * <pre>
