...
public interface IChatBaseComponent extends Iterable<IChatBaseComponent> {
	...
	class ChatSerializer implements JsonDeserializer<IChatBaseComponent>, JsonSerializer<IChatBaseComponent> {
		...
		public IChatBaseComponent a(JsonElement jsonelement, Type type,
			...
			if (jsonelement.isJsonPrimitive()) {
				...
				if (jsonelement.isJsonArray()) {
                    ...
                    for (JsonElement jsonelement1 : jsonarray) {
						...
						IChatBaseComponent ichatbasecomponent1 = this.a(jsonelement1, jsonelement1.getClass(),
				...
				if (jsonobject.has("text")) {
					...
					object = new ChatComponentScore(ChatDeserializer.h(jsonobject1, "name"),
							ChatDeserializer.h(jsonobject1, "objective"));
		...
		private void a(ChatModifier chatmodifier, JsonObject jsonobject,
		...
		public JsonElement a(IChatBaseComponent ichatbasecomponent, Type type,
				JsonSerializationContext jsonserializationcontext) {
			if (ichatbasecomponent instanceof ChatComponentText && ichatbasecomponent.getChatModifier().g()
					&& ichatbasecomponent.a().isEmpty()) {
				...
				if (!ichatbasecomponent.a().isEmpty()) {
					...
					for (IChatBaseComponent ichatbasecomponent1 : ichatbasecomponent.a()) {
						jsonarray.add(
								this.a(ichatbasecomponent1, ichatbasecomponent1.getClass(), jsonserializationcontext));
					}
				...
				if (ichatbasecomponent instanceof ChatComponentText) {
					...
					if (chatmessage.j() != null && chatmessage.j().length > 0) {
						...
						for (Object object : aobject) {
							if (object instanceof IChatBaseComponent) {
								jsonarray1.add(this.a((IChatBaseComponent) object, object.getClass(),
					...
					if (!(ichatbasecomponent instanceof ChatComponentSelector)) {
						throw new IllegalArgumentException(
								"Don't know how to serialize " + ichatbasecomponent + " as a Component");
					}
		...
		@Override
		public JsonElement serialize(IChatBaseComponent object, Type type,
				JsonSerializationContext jsonserializationcontext) {
			return this.a(object, type, jsonserializationcontext);
		}

		@Override
		public IChatBaseComponent deserialize(JsonElement jsonelement, Type type,
				JsonDeserializationContext jsondeserializationcontext) throws JsonParseException {
