...
public class BlockCactus extends Block
{
	...
	public void b(World world, BlockPosition blockposition, IBlockData iblockdata, Random random)
	{
		...
		if (world.isEmpty(blockposition1))
		{
			...
			if (i < world.paperSpigotConfig.cactusMaxHeight)
			{ // PaperSpigot - Configurable max growth height for
				// cactus blocks) {
				int j = iblockdata.get(BlockCactus.AGE).intValue();

				if (j >= (byte) range(3, (world.growthOdds / world.spigotConfig.cactusModifier * 15) + 0.5F, 15))
				{ // Spigot
