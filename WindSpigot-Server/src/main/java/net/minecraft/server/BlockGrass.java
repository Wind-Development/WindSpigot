...
public class BlockGrass extends Block implements IBlockFragilePlantElement {
	...
	public void b(World world, BlockPosition blockposition, IBlockData iblockdata, Random random) {
		if (!world.isClientSide) {
			...
			if (world.getLightLevel(blockposition.up()) < 4
					&& (lightLevel = world.getType(blockposition.up()).getBlock().p()) > 2) {
				// CraftBukkit start
				...
				if (world.tacoSpigotConfig.grassIgnoresLight || lightLevel >= 9) { // TacoSpigot - add an option to
																					// ignore light
					for (int i = 0; i < Math.min(4, Math.max(20, (int) (4 * 100F / world.growthOdds))); ++i) { // Spigot
						...
						if (iblockdata1.getBlock() == Blocks.DIRT
								...
								&& world.isLightLevel(blockposition1.up(), 4) && block.p() <= 2) {
							// CraftBukkit start
		...
		while (i < 128) {
			...
			while (true) {
				if (j < i / 16) {
					...
					if (world.getType(blockposition2.down()).getBlock() == Blocks.GRASS
