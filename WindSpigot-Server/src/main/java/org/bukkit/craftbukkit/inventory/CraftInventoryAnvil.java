...
public class CraftInventoryAnvil extends CraftInventory implements AnvilInventory {
	...
	public ItemStack getItem(int slot) {
		if (slot < getIngredientsInventory().getSize()) {
			...
			net.minecraft.server.ItemStack item = getResultInventory()
					.getItem(slot - getIngredientsInventory().getSize());
			return item == null ? null : CraftItemStack.asCraftMirror(item);
		}
	}
	...
	public void setItem(int index, ItemStack item) {
		if (index < getIngredientsInventory().getSize()) {
			...
			getResultInventory().setItem((index - getIngredientsInventory().getSize()),
